...
- `main.js` — the only place with Node/OS access. Owns:
  ...
  - `app.requestSingleInstanceLock()` at the very top of the file, before anything else runs — a second instance doesn't share or take over the first's `globalShortcut` registrations, it just silently fails to register anything the first already holds (console-only warning, invisible in the packaged exe), making the shortcuts look randomly broken with zero explanation. Confirmed directly. A rejected second instance quits immediately; `second-instance` focuses the existing window instead.
...
No tray icon, no minimize-to-tray, no auto-updater, no code signing, no custom app icon, no `dialog.showSaveDialog` Save-As flow (recordings save silently to the OS Downloads folder, same as the web version). These are deliberate omissions to keep this a small single-purpose tool — don't add them speculatively; add them if/when actually requested. (`requestSingleInstanceLock` used to be on this list too — it was added for a concrete reason, see main.js's architecture note above, not speculatively.)
